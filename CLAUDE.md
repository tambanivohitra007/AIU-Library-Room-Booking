...
- Roles: `SUPERADMIN` > `ADMIN` > `STUDENT_WORKER` > `FACULTY` = `STUDENT`. `FACULTY` carries exactly the same (non-privileged) permissions as `STUDENT` and exists only so a department manager is not labelled a student — every privilege check is an allowlist naming ADMIN/SUPERADMIN/STUDENT_WORKER, so a new base role is powerless by construction. **A `DepartmentAdmin` grant adds scope but removes none: an ADMIN with a grant still controls every department.** Confining someone means setting a base role *and* granting the department. Only SUPERADMIN can edit Service Settings, change roles, or manage admin accounts; ADMIN has full operational powers. `ADMIN_EMAILS` env entries bootstrap as SUPERADMIN on first SSO login
